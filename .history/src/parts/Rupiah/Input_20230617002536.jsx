...
const Input = () => {
    ...
    async function fnSubmit(e) {
        e.preventDefault();
        // Perform calculation using amount and exchangeRate
        const usdToIdr = 14964.35;
        // currency idr to usd
        const idrToUsd = 0.000067;

        const result = exchangeRate * idrToUsd;
        setResult(result);
        console.log("Result:", result);
        console.log("Amount:", amount);
        console.log("Exchange Rate:", exchangeRate);
        console.log("USD to IDR:", usdToIdr)
        console.log("IDR to USD:", idrToUsd)
    }
                ...
                <div className="w-full mb-10">
                    ...
                    <form className="mt-4 flex items-center justify-center gap-3" onSubmit={fnSubmit}>
                        ...
                        >
                            {isLoading ? (
                                ...
                                data
                                    ?.filter((item) =>
                                        ["BTC-USD", "ETH-USD", "DOT-USD"].includes(item.symbol)
                                    )
                                    .map((item, index) => {
                    ...
                    <div className="mt-4">
                        {isLoading ? (
                            ...
                            <p>
                                {amount} IDR is equal to {result} ETH
                            </p>
                        )}
                    </div>
                </div>
